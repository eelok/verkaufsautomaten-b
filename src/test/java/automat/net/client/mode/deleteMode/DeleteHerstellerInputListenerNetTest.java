...
class DeleteHerstellerInputListenerNetTest {
    ...
    void should_throws_exception() throws IOException, ClassNotFoundException {
        ...
        doThrow(new IOException()).when(dataSender).sendDataToServer(textToSend, Command.DELETE_HERSTELLER);

        assertThatThrownBy(() -> deleteHerstellerInputListenerNet.onInputEvent(event))
